...
from apps.grammar.models import GrammarExample, GrammarPoint


DEFAULT_JSON_DIR = Path("data") / "hsk_level updated"
DEFAULT_JSON_FILE_NAME = "hsk6_grammar_import.json"


class Command(BaseCommand):
    help = "Import grammar data from a JSON file that follows data/grammar_import_template.json."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            help="Keep old examples for updated grammar points instead of replacing them.",
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["dry_run"]:
            self.stdout.write(
                self.style.WARNING(
                    f"Dry run OK: {json_path} contains {len(data)} grammar points."
                )
            )
            return
        ...
        with transaction.atomic():
            ...
            for index, item in enumerate(data, start=1):
                ...
                grammar_point, created = self.upsert_grammar_point(
                    item, level, topic, index
                )
                if created:
                    stats["grammar_created"] += 1
                else:
                    stats["grammar_updated"] += 1

                if not options["keep_existing_examples"]:
                    grammar_point.examples.all().delete()

                examples = self.build_examples(grammar_point, item.get("examples", []))
                if examples:
                    GrammarExample.objects.bulk_create(examples)
                    stats["examples_created"] += len(examples)

        self.stdout.write(self.style.SUCCESS(f"Imported grammar from {json_path}"))
        self.stdout.write(
            "Levels created: {levels_created}; topics created: {topics_created}; "
            "grammar created: {grammar_created}; grammar updated: {grammar_updated}; "
    ...
    def validate_data(self, data, json_path):
        ...
        for index, item in enumerate(data, start=1):
            ...
            missing = [
                field
                for field in sorted(self.required_grammar_fields)
    ...
    def upsert_grammar_point(self, item, level, topic, index):
        lookup = {
            "level_hsk": level,
        ...
        defaults = {
            "topic": topic,
            "structure": self.clean_text(item["structure"]),
            "meaning_vi": self.clean_text(item["meaning_vi"]),
            "explanation": self.clean_text(item["explanation"]),
        ...
        matches = GrammarPoint.objects.filter(**lookup)
        if matches.count() > 1:
            matches = matches.filter(order_number=order_number)
        if matches.count() > 1:
            raise CommandError(
                "Multiple grammar points match HSK='{level}', title='{title}', "
                "order_number={order}.".format(
                    level=level.name,
                    title=lookup["title"],
                    order=order_number,
                )
        ...
        grammar_point = matches.first()
        if grammar_point:
            for field, value in defaults.items():
                setattr(grammar_point, field, value)
            grammar_point.save()
            return grammar_point, False

        grammar_point = GrammarPoint.objects.create(**lookup, **defaults)
        return grammar_point, True

    def build_examples(self, grammar_point, examples):
        return [
            GrammarExample(
                ...
                explanation=self.clean_nullable_text(example.get("explanation")),
